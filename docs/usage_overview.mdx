...
1. `segul` has two input options. Each have its pros and cons. See [command options](https://github.com/hhandika/segul/wiki/4.-Command-Options#input-options) for details.
2. By default, `segul` datatype is DNA. If your input file is amino acid sequences, you are required to use `--datatype aa` option.
3. Except for computing alignment statistics, you can pass `--datatype ignore`. This will speed up the processing time up to 40 percent. Use this option carefully. It is safe for DNA sequences. It is only safe for amino acid sequences when the output is not in nexus format. Learn more in the [command options](https://github.com/hhandika/segul/wiki/4.-Command-Options#data-types).
